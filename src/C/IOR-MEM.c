     ...
     char ** argv)
{
    ...
    if (rank == 0 && verbose >= VERBOSE_0) {
      ...
      fprintf(stdout, "Total run time: %f", runTime);
    }
    fin_s = GetTimeStamp();
    //KMM SCR
    SCR_Finalize();    
    ...
    fin_e = GetTimeStamp();
    if (rank == 0) {
      printf("Runtime (init run fin): %f %f %f", init_e - init_s, runTime, fin_e - fin_s);
    }
               ...
               int             access)
{
    ...
    if (verbose >= VERBOSE_3) {
        fprintf(stdout,
                "[%d] At file byte offset %lld, comparing %lu-byte transfer\n",
    ...
    for (i = 0; i < length; i++) {
        if (testbuf[i] != goodbuf[i]) {
            ...
            if (verbose >= VERBOSE_2) {
                fprintf(stdout,
          "[%d] At transfer buffer #%ld, index #%ld (file byte offset %lld):\n",
	...
	fprintf(stdout, "[%d]   In transfer %ld, ",
                rank, (long)transferCount);
	fprintf(stdout, "%ld errors between buffer indices %ld and %ld.\n",
		errorCount, first, last);
...
void
DelaySecs(int caltime)
{
    double t1,t2;
    double unit = 10;
    double c = caltime - unit;
    double total = 0;
    int i, j;
    ...
    int v = 2 * 2 * 2 * 2 * 2;
    v = v * v;
    long count  = v * v * v / sizeof(double); //1GB

    if (rank == 0 && caltime > 0 && verbose >= VERBOSE_1) {
        fprintf(stdout, "delaying %d seconds . . .\n", caltime);
    }
    //Kento ATA
    //sleep(delay);
    t1 = secs();
    sb = (double *)malloc(count * sizeof(double));
    rb = (double *)malloc(count * sizeof(double));
    for (i = 0; i < caltime; i++) {
      for (j = 0; j < count; j++) {
        rb[j] = sb[j];
      }
    ...
    //    count = caltime / (t2 - t1);
    //    while (count > 0) {
    //      MPI_Alltoall(sb, chunk, MPI_CHARACTER, rb, chunk, MPI_CHARACTER, MPI_COMM_WORLD);
    //      count--;
    //    }
    int size  = count * sizeof(double) * 2 / (1000 * 1000);
    printf("%d: MEM: Did work(size: %d MB, unt: %d MB, BW: %f MB/s): %f [s] / %d caltime\n", rank, size * caltime, size, size * caltime / (t2 - t1), t2 - t1, caltime);
...
static int myrand() { my_r = (my_r * 9631) % 21599; return my_r; }
...
DisplayUsage(char ** argv)
{
    char * opts[] = {
"OPTIONS:",
" -A N  User supplied test reference number [-1]",
...
" -Z    reordertasksrandom -- changes task ordering to random ordering [!HDF5]",
" -X N  reordertasksrandomseed - random seed for -Z (reordertasksrandom) [!HDF5]",
...
" -z    randomOffset -- access is to random, not sequential, offsets, with a file",
" ",
...
ShowTest(IOR_param_t * test)
{
    ...
    fprintf(stdout, "\t%s=%ld\n", "setAlignment", (long)test->setAlignment);
...
SummarizeResults(IOR_param_t * test)
{
   ...
   for (ival=0; ival<2; ival++)
   {
    ...
    if (rank == 0 && verbose >= VERBOSE_0) 
    {
        ...
        fprintf(stdout," %d %d %d %d %d %d %d %d %d %d %d %lld %d %s",test->numTasks,test->tasksPerNode,test->repetitions,test->filePerProc,test->reorderTasks,test->taskPerNodeOffset,test->reorderTasksRandom,test->reorderTasksRandomSeed,test->segmentCount,test->blockSize,test->transferSize,test->aggFileSizeForBW[0],test->TestNum,test->api);
	...
	/* use repetition count for number of multiple files */
	  ...
	  && (maxTimeDuration
	      ? (GetTimeStamp() - startTime < maxTimeDuration) : 1)) {
            ...
            if (test->useExistingTestFile == FALSE) {
                ...
                //SCR removing this operation because checkpoints under SCR control
            }
            MPI_CHECK(MPI_Barrier(testComm), "barrier error");
            test->open = WRITE;
            ...
            //printf(" ***** SCR_ROUTE_FILE %s\n", SCR_testFileName);

            fd = IOR_Create(SCR_testFileName, test);
		...
		MPI_CHECK(MPI_Barrier(testComm), "barrier error");
            ...
            test->aggFileSizeFromStat[rep]
                = IOR_GetFileSize(test, testComm, SCR_testFileName);
	    ...
	    if (verbose >= VERBOSE_3) WriteTimes(test, timer, rep, WRITE);
            ...
            //printf(" ***** SCR_COMPLETE_CHECKPOINT\n");
	  ...
	  && (maxTimeDuration
        ...
        if (!test->keepFile && !(test->keepFileWithError && test->errorFound)) {
            // SCR removing operation because checkpoint files under SCR control
            //RemoveFile(testFileName, test->filePerProc, test);
        }

...
DelaySecs (int caltime)
{
    ...
    do_work(c + unit);
    //    t2 = secs();
    //    printf("ATS: Did work: %f[s]/  %d[s]\n", t2-t1, caltime);

} /* DelaySecs() */

#define ARR_MAX 1024
#define LOOP_NUM (10000.0)
#define DELAY_UNIT (1000.0 * 1000.0)
	...
	/* use repetition count for number of multiple files */
	  ...
	  && (maxTimeDuration
	    ...
	    for (i = 0; i < LOOP_NUM; i++) {
	      ...
	      rk = rank;
	      MPI_Allreduce(&rk, &max, 1, MPI_INT, MPI_MAX, testComm);
	    }
	    fprintf(stdout, "%d: MPI_Allreduce(max:%d,rk:%d) \n", rank, max, rk);

            MPI_CHECK(MPI_Barrier(testComm), "barrier error");
	    ard_e = secs();
	    fprintf(stdout, "task %d Loops time: %f \n", rank, ard_e - ard_s );
	    /**/
            test->open = WRITE;
